...
| Eval with expected outcomes | Three LangSmith-backed datasets (53 examples total) with three different evaluator types — see [Evals](#evals) |
...
1. **Intent routing** (`evals/datasets/intent-routing.jsonl`, 24 examples) — exact string match against expected `route_search`/`discovery`/`knowledge` labels. No model involved in scoring; runs in seconds, cheap enough to re-run on every prompt edit. Last run: **24/24 correct**, after a real prompt fix — a bare greeting ("hi there") was misclassifying as `discovery`, which would have forced the model to fabricate search probes and burn seats.aero quota answering "hello."
2. **Search planning** (`evals/datasets/search-planning.jsonl`, 18 examples, frozen reference clock) — field-level partial credit (`setF1` on airport sets, IoU on date windows), because a plan that finds the right origin but misses one destination airport is mostly right, not simply wrong. Results are routed through the real `searchPlan` merge reducer before scoring, matching what the production graph actually produces. Last run: **average score ≈0.994** across 18 live examples (17 exact matches; one 0.947 from a defensible seasonal-convention ambiguity; one 0.90 from a confirmed, reproducible bug in `inferMultiCityRoute`'s regex fallback — flagged as a follow-up, not fixed here, since it's above threshold and a distinct root cause from this task).
3. **End-to-end groundedness** (`evals/datasets/groundedness.jsonl`, 11 examples, forced replay mode) — an LLM judge scores helpfulness (where judgment is genuinely required: does it answer the question, name a program, give a booking path, cite the knowledge base), but the hallucination check reuses the exact same deterministic `findViolations` set-membership logic as the live groundedness node — that check doesn't become less trustworthy just because it's running offline. Last run: **groundedness 1.000 — perfect across every example, on 4 independent runs (44/44 total)**. **Helpfulness averaged 0.705**, below this task's own 0.75 target, but not for the reason that number suggests: the judge's four criteria are scored independently and averaged, and `namesProgram` is correctly `false` on the dataset's deliberate "no availability found" examples (by design — the plan's own judge prompt states this) — so a perfectly honest, correct answer to those questions can only ever hit 2 of 4 criteria, which mechanically caps the average below 0.75 regardless of answer quality. Confirmed by reading the actual judged answers directly, not just the number: no example failed for a real correctness or hallucination reason.

Two real, distinct issues surfaced while building this suite and are tracked as follow-ups rather than papered over (see below).

Building this eval suite surfaced two genuine bugs unrelated to the evals themselves, both fixed in place: fixture recordings never included the `take`/`order_by` params the app always sends, so every replay-mode search was silently 404ing regardless of dataset quality (worked around for this dataset's fixtures; the recording script itself still needs the same fix — tracked as a follow-up); and a Voyage 429 rate-limit during knowledge retrieval was being treated identically to a genuine outage instead of retried, which briefly produced a worse-than-necessary degraded answer during the eval run itself.

## What I'd improve with more time

- **Discovery's origin handling.** Live testing surfaced a real, reproducible case — `"From Chicago, where should I take a weekend trip during the summer using points?"` — where the discovery planner missed an explicitly-named origin and (before a prompt fix) hallucinated bank names ("amex", "chase") as if they were searchable mileage programs. The program-hallucination half is fixed and verified; the origin-extraction miss on that specific phrasing persists after the same fix, which points to structured-output reliability on longer sentences rather than pure prompt wording — needs a retry-on-empty-origin mechanism (mirroring `plan-search.ts`'s existing retry-on-incomplete-route logic) rather than another prompt tweak. A related but distinct angle on the same area: even when an origin *is* extracted, it can resolve to a seats.aero multi-city/metro code (e.g. "CHI" for Chicago) rather than a literal airport IATA code — worth verifying every downstream consumer of `searchPlan.origins` handles synthetic multi-city codes the same way it handles real airport codes, not just the extraction step itself.
- **Fix `record-fixtures.ts` at the source**, not just the fixtures one eval dataset happened to need — see above.
...
- **Multi-turn eval coverage.** All three eval datasets are single-turn; multi-turn conversation memory (`searchPlan`'s merge reducer, added after initial development) has one hand-written regression test built from a real observed bug trace, but no dataset-driven eval coverage yet.
